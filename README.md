...
| a plot of points | trace a path through them; **one point per symbol** | One symbol, one value, at its centre, and nothing between. No path is fitted and no joining line is drawn, in the data or in any verification view. This also settles hysteresis — a loop is not a function, but as points there is no crossing to make. |
| telling scatter series apart | cluster the pixels; **group the symbols** | Pixels are the wrong unit for a scatter: every symbol carries a halo, so one series arrives as a saturated cluster plus a pale one — a magnetic panel lost every point above 100 K that way. A symbol has one stable colour, size and fill; series are groups agreeing on all three, and two colours differing only along the line toward the background are the same series drawn over itself. |
| markers deleted before tracing | text filters as written; **a marker is one of many** | Three filters were eating data. The glyph filter cannot tell a marker from a character one at a time, so the modal equant size is exempt. OCR read a row of markers as a CJK glyph and the text filter deleted a whole series tail — a text box covering ≥3 identical markers is now a misread. And the size band was anchored to a median that anti-aliasing drags down, discarding genuinely large symbols. |
| verifying a scatter | match the rendered symbols; **a dot at each centre** | Comparing re-drawn symbols against the original ink measures the pen, not the reading — an open circle's centre is background, so a perfect ring scored as off-ink. Shape-dependent gates are dropped; what remains is every point sitting on ink of its own colour, and deviation measured against the nearest symbol *centre* in its column. |
